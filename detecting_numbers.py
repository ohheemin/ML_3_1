...
def train_network(activation='relu', epochs=20, batch_size=100, learning_rate=0.1, optimizer='adam'):
    (x_train, t_train), (x_test, t_test) = load_mnist(normalize=True, one_hot_label=True)

    input_dim, hidden1_dim, hidden2_dim, output_dim = 784, 100, 50, 10
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, x_train.shape[0], batch_size):
            ...
            if optimizer == 'sgd':
                ...
                t_step = epoch * (x_train.shape[0] // batch_size) + (i // batch_size) + 1
                for i in range(6):
                    params[i], m[i], v[i] = adam_update(params[i], grads[i], m[i], v[i], t_step, learning_rate, epsilon=epsilon)
    ...
    return loss_list, acc_list, params

def preprocess_image(image_path):
    if not os.path.exists(image_path):
        raise FileNotFoundError(f"이미지 경로가 잘못되었습니다: {image_path}")
...
def predict_digit(image_path, params, activation='relu'):
...
loss_list, acc_list, trained_params = train_network(activation='relu', optimizer='adam')

image_path = "/home/ohheemin/Downloads/30000386829_1280.jpg"
pred, prob = predict_digit(image_path, trained_params, activation='relu')
print(f"\n이미지 예측 결과: {pred} (확률: {prob[0][pred]:.4f})")
